...
<?php	foreach($obj_menu->menu_bar as $v):	?>            	
                <li>
                    <a href="<?php echo esc_url( home_url( '/' ).'?'.$v->link ); ?>" data-direction="reverse" data-transition="none" data-icon="<?php echo $v->icon; ?>" <?php echo(($_SERVER['QUERY_STRING'] == $v->link)?'class="ui-btn-active ui-state-persist"':'');?>>
                        <?php echo $v->title; ?>
                    </a>
                </li>
<?php	endforeach; ?>
